...
describe("Advanced Applications", () => {
  describe("Login", () => {
    ...
    test(`[3] Filling out the login form and submitting
      ...
      expect(
        screen.queryByText(st.closuresTitle, queryOptions)
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(st.hooksTitle, queryOptions)
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(st.expressTitle, queryOptions)
      ).not.toBeInTheDocument();
      // texts not there yet
      expect(
        screen.queryByText(st.closuresText, queryOptions)
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(st.hooksText, queryOptions)
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(st.expressText, queryOptions)
      ).not.toBeInTheDocument();
      // topics not there yet
      expect(
        screen.queryByText(`Topic: ${st.closuresTopic}`, queryOptions)
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(`Topic: ${st.hooksTopic}`, queryOptions)
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(`Topic: ${st.expressTopic}`, queryOptions)
      ).not.toBeInTheDocument();
      // success message not there yet
      expect(
        screen.queryByText("Here are your articles, Foo!", queryOptions)
      ).not.toBeInTheDocument();
